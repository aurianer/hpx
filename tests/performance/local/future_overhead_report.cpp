...
// Time async action execution using wait each on futures vector
void measure_action_futures_wait_all(std::uint64_t count, const std::size_t
        steps)
{
    const hpx::naming::id_type here = hpx::find_here();
    std::vector<future<double>> futures;
    ...
    hpx::util::perf_test_report("future overhead - action - WaitAll",
            "no-executor", steps, [&]()->void
            {
                for (std::uint64_t i = 0; i < count; ++i) {
                    // For now we push back in the same vector each time
                    futures.push_back(async<null_action>(here));
                }
                wait_all(futures);
            }
...
{
    {
        ...
        if (test_all)
        {
            ...
            measure_action_futures_wait_all(count, repetitions);
#endif
