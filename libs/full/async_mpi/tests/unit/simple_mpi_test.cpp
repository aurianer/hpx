...
int hpx_main(hpx::program_options::variables_map& vm)
{
...

    {
        ...
        token = (rank == 0) ? 1 : -1;
        int rank_from = (rank + 1) % size;
        int rank_to = (rank + 1) % size;

        if (rank > 0)
        {
            // rank 1 pre-post a receive
            hpx::future<int> f_recv = hpx::async(
                    exec, MPI_Irecv, &token, 1, MPI_INT, rank_from, 0);

            // when the recv completes,
            f_recv.then([=, &exec, &token, &counter](auto&&) {
                    msg_recv(rank, size, rank_to, rank_from, token, 0);
                    --counter;
                });
        }
        ...
        if (rank == 0)
        {
            auto f_send = hpx::async(
                exec, MPI_Isend, &token, 1, MPI_INT, rank_to, 0);
            f_send.then([=, &token, &counter](auto&&) {
                msg_send(rank, size, rank_to, rank_from, token, 0);
                --counter;
            });
        }
