...
function(hpx_collect_usage_requirements target compile_definitions compile_options
  ...
  get_target_property(_target_compile_definitions ${target} INTERFACE_COMPILE_DEFINITIONS)
  get_target_property(_target_compile_options ${target} INTERFACE_COMPILE_OPTIONS)
  get_target_property(_target_pic_option ${target} INTERFACE_POSITION_INDEPENDENT_CODE)
  get_target_property(_target_include_directories ${target} INTERFACE_INCLUDE_DIRECTORIES)
  get_target_property(_target_system_include_directories ${target} INTERFACE_SYSTEM_INCLUDE_DIRECTORIES)
    ...
    foreach(dep IN LISTS _target_link_libraries)
      ...
      elseif(TARGET ${dep})
        ...
        if (NOT dep IN_LIST _already_processed_targets)
          ...
          list(APPEND _target_system_include_directories "${dep_system_include_directories}")
  ...
  set(${compile_definitions} ${_target_compile_definitions} PARENT_SCOPE)
  set(${compile_options} ${_target_compile_options} PARENT_SCOPE)
  set(${pic_option} "${_target_pic_option}" PARENT_SCOPE)
  set(${include_directories} ${_target_include_directories} PARENT_SCOPE)
  set(${system_include_directories} ${_target_system_include_directories} PARENT_SCOPE)
  set(${link_libraries} ${_libraries} PARENT_SCOPE)
  set(${link_options} ${_target_link_options} PARENT_SCOPE)
...
function(hpx_sanitize_usage_requirements property is_build)
  ...
  foreach(prop IN LISTS ${property})
    if(is_build)
      ...
      string(REPLACE "$<INSTALL_INTERFACE:" "$<1:${CMAKE_INSTALL_PREFIX}/" prop "${prop}")
    endif()
    set(_sanitized_property ${_sanitized_property} ${prop})
  endforeach()
  set(${property} ${_sanitized_property} PARENT_SCOPE)

endfunction(hpx_sanitize_usage_requirements)


# Append the corresponding (-D, -I) flags for the compilation
function(hpx_construct_cflag_list compile_definitions compile_options pic_option
    include_directories sys_include_dirs cflag_list)
...
function(hpx_generate_pkgconfig_from_target target template is_build)
  ...
  hpx_collect_usage_requirements(${target}
  ...
  foreach(component IN LISTS _component_list)
    hpx_collect_usage_requirements(${component} dep_compile_definitions dep_compile_options
      dep_pic_option dep_include_directories dep_system_include_directories
      dep_link_libraries dep_link_options processed_targets ${is_component})
    ...
    list(APPEND hpx_system_include_directories ${dep_system_include_directories})
  ...
  hpx_construct_cflag_list(hpx_compile_definitions hpx_compile_options hpx_pic_option
  ...
  configure_file(cmake/templates/${template}.pc.in
